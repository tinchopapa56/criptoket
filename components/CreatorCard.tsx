...
const CreatorCard = ({ rank, creatorImage, creatorName, creatorEths } : ICreatorCard) => {
  ...
  return (
    <div className="min-w-190 minlg:min-w-240 dark:bg-nft-black-3 bg-white border dark:border-nft-black-3 border-nft-gray-1 rounded-3xl flex flex-col p-4 m-4">
      ...
      <div className="mt-3 minlg:mt-7 text-center flexCenter flex-col">
        ...
        <p className="mt-1 font-poppins dark:text-white text-nft-black-1 font-semibold text-base">{creatorEths.toFixed(2)} <span className="font-normal">{"ETH HARD"}</span></p>
      </div>
    </div>
  );
};
