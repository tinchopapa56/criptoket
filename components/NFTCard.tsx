...
const NFTCard = ({ nft, onProfilePage }: { nft: any, onProfilePage?: boolean }) => {
  // const { nftCurrency } = useContext(NFTContext);
  const nftCurrency = "HARD";
      ...
      <div className="flex-1 min-w-215 max-w-max xs:max-w-none sm:w-full sm:min-w-155 minmd:min-w-256 minlg:min-w-327 dark:bg-nft-black-3 bg-white rounded-2xl p-4 m-4 minlg:m-8 sm:my-2 sm:mx-2 cursor-pointer shadow-md">
        ...
        <div className="mt-3 flex flex-col">
          ...
          <div className="flexBetween mt-1 minlg:mt-3 flex-row xs:flex-col xs:items-start xs:mt-3">
            <p className="font-poppins dark:text-white text-nft-black-1 font-semibold text-xs minlg:text-lg">{nft.price} <span className="normal">{nftCurrency ?? "Eth HARD"}</span></p>
