...
const Home = () => {
  ...
  const topCreators = [{
    seller: "seller TEST",
    rank: 1,
    creatorImage: "images/creator1.jpg",
    creatorName: "0x1234567890123456789012345678901234567890",
    creatorEths: "100",
    sum: 1.5,
    tokenId: 1
  }, {
  ...
  ] //getCreators(nftsCopy);

  const onHandleSearch = (value: any) => {
    // const filteredNfts = nfts.filter(({ name }) => name.toLowerCase().includes(value.toLowerCase()));
    const filteredNfts = nfts.filter((nft: any) => nft.name.toLowerCase().includes(value.toLowerCase()));
...
interface IHotBids {
  activeSelect: string,
  setActiveSelect: (param: string) => any,
  onHandleSearch: (param: string) => any,
  onClearSearch: () => any
}
const HotBids = ({ activeSelect, setActiveSelect, onHandleSearch, onClearSearch }: IHotBids) => (
  <div className="mt-10">
